...
libreq(MCPanel, glmnet, ggplot2, latex2exp, plot.matrix)
## Reading data
...
sim_run = function(Y, nr = 10, t0 = 10, N_t = 8, assignment = c("simul", "stagg"),
    verbose = FALSE){
  NN = nrow(Y); assignment = match.arg(assignment)
  # generate mask for missingness
  treat_mat = matrix(1L, NN, T);
  treat_indices = sample(1:NN, N_t)
  missing_mask = switch(assignment,
    stagg = stag_adapt(Y, N_t, t0, treat_indices),
    simul = simul_adapt(Y, N_t, t0, treat_indices)
  )
  if(verbose) plot(missing_mask)
...
for (i in c(1:num_runs)) {
  ...
  for (j in c(1:length(T0))) {
    treat_mat = matrix(1L, N, T); t0 = T0[j]
    ## Simultaneuous (simul_adapt) or Staggered adoption (stag_adapt)
    if (is_simul == 1) { treat_mat = simul_adapt(Y, N_t, t0, treat_indices)
    } else { treat_mat = stag_adapt(Y, N_t, t0, treat_indices) }
    Y_obs = Y * treat_mat
    ## ------
    ## MC-NNM
    ## ------
...
df1 =
  structure(
    list(
      ...
      lb = c(
        DID_avg_RMSE - 1.96 * DID_std_error, EN_avg_RMSE - 1.96 * EN_std_error,
      ...
      Method = c(
        replicate(length(T0), "DID"), replicate(length(T0), "EN"),
        replicate(length(T0), "EN-T"), replicate(length(T0), "MC-NNM"),
        replicate(length(T0), "SC-ADH")
      ),
      Marker = c(
        replicate(length(T0), 1), replicate(length(T0), 2),
...
if (to_save == 1) {
  ...
  colnames(df2) = c(
    "N", "T", "N_t", "is_simul", replicate(length(T0), "DID"),
