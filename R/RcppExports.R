...
#' @param to_estimate_v Optional boolean input for wheter estimating fixed time effects (column means of M) or not. Default is 1.
...
mcnnm_lam_range = function(M, mask,
  ...
  .Call('_MCPanel_mcnnm_lam_range', PACKAGE = 'MCPanel',
    M, mask,
    to_estimate_u, to_estimate_v,
    niter, rel_tol)
}
...
mcnnm = function(M, mask, num_lam_L = 100L, lambda_L = as.numeric(c()),
  ...
  .Call('_MCPanel_mcnnm', PACKAGE = 'MCPanel',
    M, mask, num_lam_L, lambda_L,
    to_estimate_u, to_estimate_v,
    niter, rel_tol, is_quiet)
}
...
mcnnm_fit = function(M, mask, lambda_L,
    to_estimate_u = 1L, to_estimate_v = 1L,
    niter = 1000L, rel_tol = 1e-5, is_quiet = 1L) {
  .Call('_MCPanel_mcnnm_fit', PACKAGE = 'MCPanel',
    M, mask, lambda_L,
    to_estimate_u, to_estimate_v,
    niter, rel_tol, is_quiet)
}
...
#' @param M Matrix of observed entries. The input should be N (number of units) by T (number of time periods).
#' @param mask Binary mask with the same shape as M containing observed entries. This is a missingness mask (not a treatment matrix: compute it as 1-W for treatment matrix W).
#' @param to_estimate_u Optional boolean input for wheter estimating fixed unit effects (row means of M) or not. Default is 1.
#' @param to_estimate_v Optional boolean input for wheter estimating fixed time effects (column means of M) or not. Default is 1.
#' @param num_lam_L Optional parameter on the number of lambda_Ls to consider for learning. The default number is 100 and lambda_L values are from minimum number which makes L zero to 1e-3 times this minimum number.
...
mcnnm_cv = function(
    M, mask,
    to_estimate_u = 1L, to_estimate_v = 1L,
    num_lam_L = 100L, niter = 400L, rel_tol = 1e-5,
    cv_ratio = 0.8, num_folds = 5L, is_quiet = 1L) {

  .Call('_MCPanel_mcnnm_cv', PACKAGE = 'MCPanel',
    M, mask,
    to_estimate_u, to_estimate_v,
    num_lam_L, niter, rel_tol, cv_ratio, num_folds, is_quiet)
...
mcnnm_wc_lam_range = function(
    ...
    to_add_ID = 1L, niter = 1000L, rel_tol = 1e-5
  ) {
  .Call('_MCPanel_mcnnm_wc_lam_range', PACKAGE = 'MCPanel',
    M, X, Z, mask,
    to_normalize, to_estimate_u, to_estimate_v, to_add_ID,
    niter, rel_tol
  )
}
...
mcnnm_wc = function(M, X, Z, mask, num_lam_L = 30L,
    ...
    lambda_H = as.numeric(c()), to_normalize = 1L,
    to_estimate_u = 1L, to_estimate_v = 1L, to_add_ID = 1L,
    niter = 100L, rel_tol = 1e-5, is_quiet = 1L) {
  .Call('_MCPanel_mcnnm_wc',
    PACKAGE = 'MCPanel',
    M, X, Z, mask,
    num_lam_L, num_lam_H, lambda_L, lambda_H,
    to_normalize, to_estimate_u, to_estimate_v, to_add_ID,
    niter, rel_tol, is_quiet)
}
...
mcnnm_wc_fit = function(M, X, Z, mask,
    lambda_L, lambda_H,
    to_normalize = 1L, to_estimate_u = 1L, to_estimate_v = 1L,
    to_add_ID = 1L, niter = 100L, rel_tol = 1e-5, is_quiet = 1L) {
  .Call('_MCPanel_mcnnm_wc_fit', PACKAGE = 'MCPanel',
    M, X, Z, mask, lambda_L, lambda_H,
    to_normalize, to_estimate_u, to_estimate_v, to_add_ID,
    niter, rel_tol, is_quiet)
}
...
# %%
#' predict complete data matrix from matrix factorization model
#' @param m mcnnm model fit
#' @param unit_fe boolean for whether to include unit F (default TRUE)
#' @param time_fe boolean for whether to include time FE (default TRUE)
#' @return predicted matrix Y(0)
#' @export
predict_mc = function(m, unit_fe = TRUE, time_fe = TRUE) {
  Yhat = m$L
  if (unit_fe) Yhat = Yhat + replicate(ncol(m$L), m$u)
  if (time_fe) Yhat = Yhat + t(replicate(nrow(m$L), m$v))
  Yhat
}
